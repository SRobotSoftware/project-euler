...
function product(var1, var2, var3, var4) {
...
function calculateGrid(data) {
	...
	data.forEach(function (x) {
		for (var i = 0; i < x.length - 3; i++) {
			var test = product(x[i], x[i + 1], x[i + 2], x[i + 3]);
	...
	for (var i = 0; i < data.length - 3; i++) {
		for (var j = 0; j < data[i].length; j++) {
			var test = product(data[i][j], data[i + 1][j], data[i + 2][j], data[i + 3][j]);
			if (test > result) result = test;
		}
	}
	// Calculate diagonal up left to down right
	for (var i = 0; i < data.length - 3; i++) {
